"""
Trust gate injection-ratio sweep.
Run from STEM-GNN/ root:
  python STEM-GNN/scripts/trust_sweep.py
"""
...
# ─── Cora injection sweep ─────────────────────────────────────────────────
def run_cora_sweep(data_dir, seeds=3, epochs=200):
    print("\n=== CORA injection sweep ===")
    ...
    for r in RATIOS:
        ei = inject(ec, tf, labels, r)
        print(f"\n--- ratio={r:.0%}  edges={ei.size(1)} ---", flush=True)
        for g in GATES:
            t2  = tf if g == "D_llm" else None
            acc = float(np.mean([
                train_eval(g, tf, ei, t2, labels, trm, vm, tem, nc, s, epochs)
...
# ─── Texas + Wisconsin heterophily ────────────────────────────────────────
def run_heterophily(webkb_root, seeds=3, epochs=200):
    print("\n=== Heterophily (Texas + Wisconsin) ===")
    out = {}
    for ds_name in ["Texas", "Wisconsin"]:
...
# ─── PubMed (larger, more OOD signal) ─────────────────────────────────────
def run_pubmed_sweep(data_dir, seeds=2, epochs=200):
    ...
    for r in RATIOS:
        ei = inject(ec, tf, labels, r)
        print(f"\n--- PubMed ratio={r:.0%}  edges={ei.size(1)} ---", flush=True)
        for g in GATES:
            t2  = tf if g == "D_llm" else None
            acc = float(np.mean([
                train_eval(g, tf, ei, t2, labels, trm, vm, tem, nc, s, epochs)
...
if __name__ == "__main__":
    ...
    cora = run_cora_sweep(DATA_DIR, seeds=SEEDS, epochs=EPOCHS)
    all_results["cora_sweep"] = cora

    hetero = run_heterophily(WEBKB_DIR, seeds=SEEDS, epochs=EPOCHS)
    all_results["heterophily"] = hetero

    pubmed = run_pubmed_sweep(DATA_DIR, seeds=2, epochs=EPOCHS)
    all_results["pubmed_sweep"] = pubmed

    out_path = "/home/lam23005/STEM-GNN/sweep_results.json"
