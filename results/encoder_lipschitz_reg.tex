...
\section{Baseline Hyperparameter Optimization (Optuna)}
\label{sec:optuna}
...
\paragraph{Implementation.} \path{results/hpo/optuna_baseline.py} drives
the search. It builds the same parameter dictionary that
\path{finetune.py}'s command-line entry point would build (Cora defaults
from \texttt{config/finetune.yaml}), then imports and calls
...
is configured with its default TPE sampler and, importantly, persists
every trial to an on-disk SQLite database
(\path{results/hpo/optuna_baseline.db}). This makes the search resumable:
if the process is interrupted, re-running the same script loads the
existing study and only runs the remaining trials rather than starting
over. \emph{(This safeguard was added after two earlier long-running
background experiments in this session were silently killed by the
sandbox environment recycling mid-run, losing all unsaved progress.)}
...
best-found hyperparameters are then re-evaluated once under the same
protocol as Section~\ref{sec:results} (all 10 splits, 60 epochs,
early-stop patience 30) to get a number directly comparable to the
baseline row of that table.

\paragraph{Results.} The 12-trial search (TPE sampler) found its best
single-split validation accuracy ($73.4\%$) at
\texttt{finetune\_lr}$\,\approx 3.47\times 10^{-4}$,
\texttt{dropout}$\,\approx 0.014$, \texttt{normalize}$\,=\,$\texttt{none}
-- close to the default learning rate ($5\times 10^{-4}$), a much lower
dropout than the default ($0.15$), and batch normalization turned off
(the default has it on). Re-evaluating that configuration under the full
10-split protocol (60 epochs, patience 30) gives:
...
\emph{Tuning did not meaningfully improve on the untuned baseline.} Every
column differs from the untuned row by less than $0.1$ points on the mean,
well inside one standard deviation. The one visible effect is a modestly
tighter val/test spread across splits (val std $3.34\to2.85$, test std
$2.10\to1.78$), consistent with the lower dropout and no batch
normalization producing slightly more consistent convergence -- but this
is a small effect on top of an already-tight baseline, not a result that
would change any conclusion in Section~\ref{sec:results}. Practically,
this says the \texttt{config/finetune.yaml} Cora defaults were already a
reasonable operating point for this checkpoint and epoch budget, and the
head-vs.-encoder regularization comparison above is not an artifact of a
