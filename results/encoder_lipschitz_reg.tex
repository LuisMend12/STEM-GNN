...
\path{scripts/homophily_shift_ood.py}). \texttt{degree\_shift\_ood.py} has
since been run and is reported immediately below; the others were not run
for this memo due to the single-CPU compute budget available. The variance
reduction observed above is suggestive but is not, by itself, a
substitute for that evaluation.

\section{Testing the Propagation-Aware Hypothesis: Degree-Shift Robustness}
\label{sec:degree-ood}

\paragraph{Motivation.} For \texttt{MySAGEConv}'s mean aggregation on a
connected undirected graph, the aggregation operator is similar to a
symmetric normalized adjacency matrix, so its spectral norm is $\le 1$:
propagation never amplifies a perturbation, at best it damps one. That
damping shrinks toward $1$ (vanishes) exactly under degree shift --
low-degree and near-isolated nodes have little neighborhood to average
over, so the aggregation step does little to attenuate whatever the
encoder's weight matrices do to the input. This gives a concrete,
structural reason to expect \texttt{encoder\_lip\_coeff} to matter
specifically here, in a way it should not for clean i.i.d.\ accuracy
(Section~\ref{sec:results}) or the feature-only shift tested in
Section~\ref{sec:tta}. This section tests that hypothesis directly rather
than leaving it as a plausible-sounding argument.

\paragraph{Experimental setup.} \texttt{scripts/degree\_shift\_ood.py}
buckets Cora's nodes by degree into ID (15th--85th percentile), OOD-low,
and OOD-high, trains only on ID nodes, and reports accuracy on all three
buckets using the best-validation-epoch weights (this script snapshots
and restores those weights explicitly, unlike \texttt{finetune.py}'s
\texttt{EarlyStopping}, which only tracks the best metrics, not the
weights themselves). Same reduced budget as the rest of this memo: the
\texttt{default} non-MoE checkpoint at epoch 50, 60 finetune epochs,
early-stop patience 30, and the same four
$(\lambda_{\text{dec}}, \lambda_{\text{enc}})$ configurations as
Section~\ref{sec:results}. \texttt{repeat=10} was set explicitly (the
script's number of seeded runs). One practical note: this script imports
\texttt{dataset}/\texttt{model}/\texttt{utils}/\texttt{task} as top-level
packages but, unlike some sibling scripts, does not add the package root
to \texttt{sys.path} itself, so it needs
\texttt{PYTHONPATH=.../STEM-GNN/STEM-GNN} set manually to run at all.
...
\midrule
Baseline (no regularization)   & \score{83.76}{1.65} & \score{78.99}{1.05} & \score{82.19}{1.29} \\
Head-only                      & \score{83.63}{1.94} & \score{78.77}{0.93} & \score{82.00}{1.27} \\
Encoder-only                   & \score{80.86}{1.02} & \score{76.08}{1.17} & \score{79.75}{1.03} \\
Head + Encoder (combined)      & \score{81.06}{0.75} & \score{76.03}{1.63} & \score{79.66}{1.06} \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{The hypothesis is not supported.} Head-only regularization
again changes nothing measurable, consistent with every earlier section.
Encoder-only regularization at $\lambda_{\text{enc}}=10^{-3}$, the same
coefficient used everywhere else in this memo, drops \emph{all three}
buckets by roughly the same amount (ID $-2.90$, OOD-low $-2.91$, OOD-high
$-2.44$ points) rather than selectively protecting the degree-shifted
buckets. Checking the ID-minus-OOD gap directly, which is the quantity
the propagation-damping argument actually predicts should shrink: the
baseline's ID$-$OOD-low gap is $4.77$ points; the encoder-only gap is
$4.78$ points -- unchanged. The ID$-$OOD-high gap moves from $1.57$ to
$1.11$ points, but that is well within the $\sim\!1$--$1.3$-point standard
deviations on these numbers, not a signal. Combining with the head-only
penalty does not rescue this: the combined row tracks the encoder-only row
closely on every column. \textbf{This is a uniform capacity penalty, not
an OOD-selective robustness benefit.}

A plausible reason this differs from the earlier sections' results: this
script trains on only the ID bucket, roughly $946$ nodes out of Cora's
$2708$ (versus the $\sim\!1350$-node train split used in
Section~\ref{sec:results}'s standard splits), so the same
$\lambda_{\text{enc}}=10^{-3}$ constrains a model fit to less data and
may simply be too strong a coefficient for this specific training-set
size, rather than the propagation-damping argument being wrong in
principle. The natural next step is a coefficient sweep (e.g.\
$10^{-4}$, $10^{-5}$) on this exact degree-bucketed setup, to check
whether a weaker constraint reveals the hypothesized OOD-selective effect
once it is no longer swamped by generic under-fitting -- that sweep has
not been run yet.
...
\subsection{Which hyperparameter actually matters? (48-trial follow-up)}
\label{sec:importance}

The 12-trial searches above disagreed on \emph{which} hyperparameter
matters most: applying Optuna's importance evaluators to its own 12-trial
study ranked \texttt{dropout} far ahead of the other two
(fANOVA $0.703$, MeanDecreaseImpurity $0.663$), while the same
random-forest-based analysis applied to Hyperopt's independent 12-trial
study ranked \texttt{normalize} highest instead ($0.646$) with
\texttt{dropout} a distant second ($0.211$). With only 12 points spread
over 3 dimensions, \texttt{dropout} and \texttt{normalize} were not
well-decorrelated in either sample, so a single search's importance
ranking was not trustworthy.

To resolve this, the same Optuna study was extended from 12 to 48 trials
(resuming the identical SQLite-backed study, same search space and
per-trial budget as Section~\ref{sec:optuna}). At 48 trials, both
importance evaluators now agree:
...
\midrule
\texttt{dropout}      & $0.595$ & $0.552$ \\
\texttt{normalize}    & $0.367$ & $0.304$ \\
\texttt{finetune\_lr} & $0.038$ & $0.144$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Dropout is the most important of the three}, with
\texttt{normalize} a clear second and \texttt{finetune\_lr} consistently
least important -- this resolves the 12-trial disagreement in Optuna's
original favor, not Hyperopt's. The \texttt{normalize} effect is real and
sizable on its own: across the 48 trials, \texttt{normalize=none} averages
$73.4\%$ single-split validation accuracy versus $66.8\%$ for
\texttt{normalize=batch} ($37$ vs.\ $11$ trials), a $6.6$-point gap. One
caveat carries over even at $n{=}48$: \texttt{dropout} and
\texttt{normalize} still show a moderate correlation ($0.58$) across the
sampled trials, because Optuna's TPE sampler concentrates later trials
near previously-promising regions rather than sampling independently --
this is not a factorial design, just a much less noisy one than $n{=}12$.

\paragraph{But the extra tuning still doesn't beat the untuned baseline.}
Re-evaluating the new best configuration
(\texttt{finetune\_lr}$\,\approx 1.29\times10^{-4}$,
\texttt{dropout}$\,\approx 0.088$, \texttt{normalize}$\,=\,$\texttt{none})
under the full 10-split protocol gives train \score{98.50}{4.27}, val
\score{76.02}{3.23}, test \score{75.87}{2.04} -- statistically
indistinguishable from the untuned defaults (test \score{75.87}{2.10}) and
no better than the earlier 12-trial-tuned result (test \score{75.91}{1.78}).
This is the interesting part: \texttt{dropout} clearly dominates
\emph{variance in the single-split, 30-epoch search objective} used to
rank configurations, but that dominance does not carry over to a
measurable difference in final all-splits, 60-epoch test accuracy. Put
plainly, correctly identifying the most important hyperparameter for the
reduced-budget search protocol did not translate into a better model
under the protocol this memo actually reports results on -- a caution
against reading too much into HPO importance analysis performed under a
cheaper proxy budget than the final evaluation.
